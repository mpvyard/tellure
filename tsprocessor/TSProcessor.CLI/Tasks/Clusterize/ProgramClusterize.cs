...
namespace TSProcessor.CLI.Tasks.Clusterize
{
    static class Clusterizer
    {
        ...
        //For Vector3
        public static int Clusterize3(ClusterizationOptions opts, ILogger logger)
        {
            logger.LogInformation("Operation started...");
            ...
            logger.LogInformation("Start clusterization");
            ClusterizeAllParallel3(series, opts.From, opts.To, logger);
            logger.LogInformation("Operation completed");
            return 0;
        }

        private static void ClusterizeAllParallel3(Series series, int[] from, int[] to, ILogger logger)
        {
            Wishart.GenerateTemplateForWishart(from, to).AsParallel().ForAll(template =>
            {
                string stringTemplate = $"{template[0]}-{template[1]}-{template[2]}-{template[3]}";
                var vectors = FindZVectors3(series, template, 0).ToList();

                logger.LogInformation("Clusterize for template {template}", stringTemplate);
                var clusters = Clusterize3(vectors);
                ...
                var centers = clusters.Select(cluster => cluster.Centr3.List3).ToList();
                var pathCenters = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory),
                    "clusters", $"{stringTemplate}.centers.json");
                using (var writer = new StreamWriter(pathCenters))
                {
        ...
        private static IEnumerable<ZVector> FindZVectors3(Series series, int[] step, int firstNumber)
        {
            List<Vector3> teachSeries = series.GetTeachSeries3();
            IEnumerable<ZVector> zVectors = ZVectorBuilder.Build3(teachSeries, step, firstNumber);
            return zVectors;
        }

        private static IEnumerable<InitialCluster> Clusterize3(List<ZVector> zVectors)
        {
            IClusterizeAlgor algor = new WishartAlgor(new WishartParams { H = 0.2, K = 11 });
            List<InitialCluster> clusters = algor.Clusterize3(zVectors);
            return clusters;
        }
        //
    }
